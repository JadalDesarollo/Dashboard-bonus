...
function TransactionPending() {
    ...
    const filterData = async (e: any) => {
        e.preventDefault()
        let filteredData = []
        const { data } = await fetchTransactionsPending(idClient)
        if (nameFilter.trim() === "") {
            getTransactions()
            return
        }
        filteredData = data.filter((row: TransactionPending) => {
...
`}
          ...
          >
            {">>"}
                            ...
                            <span>
                                Cantidad de transacciones pendientes: <strong>{data.length}</strong>
                            </span>
                        </div>
